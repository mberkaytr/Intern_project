...
/* ---------------------- Admin Login Check ----------------------------------- */
    ...
    public function change_password_for_employee($data){
		...
		if($password == $re_password){
			...
			$message = 'Üzgünüm !! Şifre Eşleşemiyor';
            return $message;
		}
	...
	public function add_new_user($data){
		...
		try{
			...
			if($total_email != 0 && $total_username != 0){
				$message = "E-posta ve Şifre zaten alınmış";
            	return $message;

			}elseif($total_username != 0){
				$message = "Kullanıcı adı önceden alınmış.";
            	return $message;

			}elseif($total_email != 0){
				$message = "E-posta Zaten önceden alınmış.";
            	return $message;
...
/* -------------admin_password_change------------*/
	...
	public function admin_password_change($data, $id){
		...
		try{
			...
			$all_error = '';
			if($total_row == 0){
				$all_error = "Geçersiz eski şifre";
			}
			

			if($admin_new_password != $admin_cnew_password ){
				$all_error .= '<br>'."Yeni Şifreleriniz eşleşmiyor.";
			}

			$password_length = strlen($admin_raw_password);

			if($password_length < 6){
				$all_error .= '<br>'."Şifre uzunluğu 6 karakterden fazla olmalıdır";
			}
	...
	public function add_new_task($data){
		...
		try{
			...
			$_SESSION['Task_msg'] = 'İş Başarıyla Eklendi';
		...
		public function update_task_info($data, $task_id, $user_role){
			...
			try{
				...
				$_SESSION['Task_msg'] = 'İş Başarıyla Güncellendi';
